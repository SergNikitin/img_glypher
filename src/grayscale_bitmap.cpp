...
static const uint8_t MAX_GREY_LEVELS = 0xFF;
...
GrayscaleBitmap::GrayscaleBitmap(SDL_Surface& givenSurface)
    ...
    // size_t grayscalePixelCount = rows*columns;
    // pixels(new pixels_vector(grayscalePixelCount, 0));
    ...
    for (int16_t thisRow = 0; thisRow < rows; ++thisRow) {
        for (int16_t thisColumn = 0; thisColumn < columns; ++columns) {
            uint8_t* rgbPixel = (uint8_t*)surface->pixels + thisRow * rows
